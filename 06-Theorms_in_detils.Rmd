...
It must be shown that the function \( F \) defined as above is actually
analytic on \( \Gamma \). Given a point \( z_{0} \in \Gamma \), choose
Jordan arcs \( C_{1} \subset D \) and \( C_{2} \subset \Delta \) with
common endpoints on \( \Gamma \) whose union \( C = C_{1} \cup C_{2} \)
is a Jordan curve enclosing \( z_{0} \). Let \( \gamma \) be the subarc
of \( \Gamma \) inside \( C \). Let \( D^{*} \) and \( \Delta^{*} \) be
the subdomains of \( D \) and \( \Delta \) enclosed by the Jordan curves
\( J_{1} = C_{1} \cup \gamma \) and \( J_{2} = C_{2} \cup \gamma \),
respectively.
![Image Credit: Duren's book](fig/pic 7.png)
