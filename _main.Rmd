...
be the quotient topological space, with projection map \(\pi : \mathbb{C} \to R.\) We will define an atlas on \(R\) that makes it a Riemann surface. Define \(U = \{\, z \in \mathbb{C} : -\pi < \operatorname{Im} z < \pi \,\}.
\) Restrict to \(U\) for convenience. Let  \(W = \pi(U) \subset R.\) Let \(\sigma : W \to U\) be the unique right inverse of \(\pi\) on \(W\), i.e. the unique map satisfying:
